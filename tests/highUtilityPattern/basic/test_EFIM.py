...
class EFIM(_ab._utilityPatterns):
    ...
    **Credits:**
    ...
    def _backTrackingEFIM(self, transactionsOfP: list, itemsToKeep: list, itemsToExplore: list,
...
if __name__ == "__main__":
    ...
    if len(_ab._sys.argv) == 5:
        ...
        oFile = _ab._sys.argv[3]
        sep = _ab._sys.argv[4]
        if sep == "\\t":
            sep = "\t"
        print("Input File:", iFile)
        print("Minimum Utility Value:", minUtil)
        print("Output File:", oFile)
        print("Separator:", sep)
        _ap = EFIM(iFile=iFile, minUtil=minUtil, sep=sep)
        _ap.mine()
        _ap.save(oFile)
        ...
        _ap = EFIM(iFile=iFile, minUtil=minUtil)
        _ap.mine()
        _ap.save(oFile)
