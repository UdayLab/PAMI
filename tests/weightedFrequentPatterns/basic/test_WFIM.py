...
class _Tree:
    ...
    @staticmethod
    def getConditionalTransactions(ConditionalPatterns: List[List[str]], conditionalFreq: List[int]) -> Tuple[
        List[List[str]], List[int], Dict[str, int]]:
        """
        ...
        for tr in self.__Database:
            for i in range(0, len(tr)):
                if tr[i] not in self.__mapSupport:
                    ...
                    self.__mapSupport[tr[i]] += 1
        self.__mapSupport = {k: v for k, v in self.__mapSupport.items() if
                             v >= self._minSup and v * _maxWeight > self._minSup}
...
if __name__ == "__main__":
    ...
    if len(_fp._sys.argv) == 7:
        ...
        wFile = _fp._sys.argv[2]
        minSup = _fp._sys.argv[3]
        minWeight = _fp._sys.argv[4]
        oFile = _fp._sys.argv[5]
        sep = _fp._sys.argv[6]
        if sep == "\\t":
            sep = "\t"
        print("Input File:", iFile)
        print("Weight File:", wFile)
        print("Minimum Support List:", minSup)
        print("Minimum Weight List:", minWeight)
        print("Output File:", oFile)
        print("Separator:", sep)
        _ap = WFIM(iFile=iFile, wFile=wFile, minSup=minSup, minWeight=minWeight, sep=sep)
        _ap.mine()
        _ap.save(oFile)
        ...
        _ap = WFIM(iFile=iFile, wFile=wFile, minSup=minSup, minWeight=minWeight)
        _ap.mine()
        _ap.save(oFile)
        ...
        raise WrongNumberOfArguments(
            "Please provide five arguments: iFile, wFile, expSup expWSup and oFile \n""or Please provide three arguments: iFile, wFile, expSup expWSup, oFile and sep")
