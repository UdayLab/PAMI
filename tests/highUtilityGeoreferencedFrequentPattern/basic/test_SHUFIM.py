    ...
    def mine(self):
        ...
        for idx, item in enumerate(_itemsToKeep):
            ...
            if self._newNamesToOldNames[item] in self._Neighbours:
                neighbors = [self._oldNamesToNewNames[y] for y in self._Neighbours[self._newNamesToOldNames[item]] if
                             y in self._oldNamesToNewNames]
                ...
                if item in itemsToKeep:
                    ...
                    if self._newNamesToOldNames[item] in self._Neighbours:
                        ...
                        for k in range(i, length):
                            ...
                            if self._newNamesToOldNames[
                                transaction_item] in itemNeighbours and transaction_item in neighbourhoodList:
...
if __name__ == "__main__":
    ...
    if len(_ab._sys.argv) == 7:
        ...
        nFile = _ab._sys.argv[2]
        minUtil = _ab._sys.argv[3]
        minSup = _ab._sys.argv[4]
        oFile = _ab._sys.argv[5]
        sep = _ab._sys.argv[6]
        if sep == "\\t":
            sep = "\t"
        print("Input File:", iFile)
        print("Neighborhood File:", nFile)
        print("Minimum Utility:", minUtil)
        print("Minimum Support:", minSup)
        print("Output File:", oFile)
        print("Separator:", sep)
        _ap = SHUFIM(iFile=iFile, nFile=nFile, minUtil=minUtil, minSup=minSup, sep=sep)
        _ap.mine()
        _ap.save(oFile)
        ...
        _ap = SHUFIM(iFile=iFile, nFile=nFile, minUtil=minUtil, minSup=minSup)
        _ap.mine()
        _ap.save(oFile)
        ...
        raise WrongNumberOfArguments(
            "Please provide five arguments: iFile, nFile, minUtil, minSup and oFile \n""or Please provide four arguments: iFile, minSup, oFile and sep")
