...
class HUFIM(_ab._utilityPatterns):
    ...
    **Credits:**
    ...
    def _backTrackingHUFIM(self, transactionsOfP: List[_Transaction], itemsToKeep: List[int], itemsToExplore: List[int],
    ...
    def _useUtilityBinArraysToCalculateUpperBounds(self, transactionsPe: List[_Transaction], j: int,
                                                   itemsToKeep: List[int]) -> None:
        """
...
if __name__ == "__main__":
    ...
    if len(_ab._sys.argv) == 6:
        ...
        minUtil = _ab._sys.argv[2]
        minSup = _ab._sys.argv[3]
        oFile = _ab._sys.argv[4]
        sep = _ab._sys.argv[5]
        if sep == "\\t":
            sep = "\t"
        ...
        print("Separator:", sep)
        _ap = HUFIM(iFile=iFile, minUtil=minUtil, minSup=minSup, sep=sep)
        _ap.mine()
        _ap.save(oFile)
        ...
        _ap = HUFIM(iFile=iFile, minUtil=minUtil, minSup=minSup)
        _ap.mine()
        _ap.save(oFile)
        ...
        raise WrongNumberOfArguments(
            "Please provide four arguments: iFile, minUtil, minSup and oFile \n""or Please provide four arguments: iFile, minSup, oFile and sep")
