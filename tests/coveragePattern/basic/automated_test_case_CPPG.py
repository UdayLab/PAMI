...
from automated_test_CPPG import test_pami
import warnings
...
class TestExample(unittest.TestCase):
    def test_num_patterns(self):
        for _ in range(3):
            ...
            cppg = test_pami(dataset)
            # As we don't have a second method to compare, we just verify the length of cppg
            self.assertGreater(len(cppg), 0, "No patterns were generated by CPPG")
    ...
    def test_equality(self):
        for _ in range(3):
            ...
            cppg_patterns = sorted(list(cppg["Patterns"]))
            self.assertTrue(len(cppg_patterns) > 0, "No patterns were generated by CPPG")
    ...
    def test_support(self):
        for _ in range(3):
            ...
            cppg.sort_values(by="Support", inplace=True)
            ps = list(cppg["Support"])
            for support in ps:
