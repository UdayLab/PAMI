...
class Apriori(_ab._frequentPatterns):
    ...
    **Calling from a python program**
    ...
    def getPatternsAsDataFrame(self) -> _ab._pd.DataFrame:
        ...
        dataFrame = _ab._pd.DataFrame(list([[" ".join(x), y] for x, y in self._finalPatterns.items()]),
...
if __name__ == "__main__":
    ...
    if len(_ab._sys.argv) == 5:
        ...
        _ap = Apriori(iFile=iFile, minSup=minSup, sep=sep)
        _ap.mine()
        _ap.save(oFile)
        ...
        _ap = Apriori(iFile=iFile, minSup=minSup)
        _ap.mine()
        _ap.save(oFile)
...
#minSupList = list(map(int, _sys.argv[2].split(',')))
