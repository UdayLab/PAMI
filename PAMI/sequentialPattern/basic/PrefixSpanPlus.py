...
class PrefixSpanPlus(_ab._sequentialPatterns):
    """
        ...
        -------------------------------
            ...
            ------
             basic:
                python3 PrefixSpanPlus.py <inputFile> <outputFile> <minSup>
             length:
                python3 PrefixSpanPlus.py <inputFile> <outputFile> <minSup> "<sep(default="\t")>" <maxLength>
             gap:
                python3 PrefixSpanPlus.py <inputFile> <outputFile> <minSup> <sep(default="\t")> <maxLength(default=float("inf"))> <maxGap>
            Examples:
            ---------
            basic
                python3 PrefixSpanPlus.py sampleDB.txt patterns.txt 10.0   (minSup will be considered in times of minSup and count of database transactions)
                python3 PrefixSpanPlus.py sampleDB.txt patterns.txt 10     (minSup will be considered in support count or frequency)
            length
                python3 PrefixSpanPlus.py sampleDB.txt patterns.txt 10 "\t" 2 (find the pattern that have two or less itemsets and ten or more support)
            gap
                python3 PrefixSpanPlus.py sampleDB.txt patterns.txt 10 "\t" float("inf") 3
            length and gap
                python3 PrefixSpanPlus.py sampleDB.txt patterns.txt 10 2
        Sample run of the importing code:
        ---------------------------------
            import PAMI.sequentialPattern.basic.PrefixSpanPlus as alg
            obj = alg.PrefixSpanPlus(iFile, minSup,sep,maxGap,maxLength,sepSeq)
            obj.mine()
...
if __name__ == "__main__":
    ...
    if len(_ab._sys.argv) >= 4 and len(_ab._sys.argv) <= 7:
        ...
        if len(_ab._sys.argv) == 6:
            _ap = PrefixSpanPlus(_ab._sys.argv[1], _ab._sys.argv[3], _ab._sys.argv[4],_ab._sys.argv[5])
        if len(_ab._sys.argv) == 5:
            _ap = PrefixSpanPlus(_ab._sys.argv[1], _ab._sys.argv[3], _ab._sys.argv[4])
        if len(_ab._sys.argv) == 4:
            _ap = PrefixSpanPlus(_ab._sys.argv[1], _ab._sys.argv[3])
        _ap.mine()
