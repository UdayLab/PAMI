...
#      (at your option) any later version.#  Copyright (C)  2021 Rage Uday Kiran
#
#      This program is free software: you can redistribute it and/or modify
#      it under the terms of the GNU General Public License as published by
#      the Free Software Foundation, either version 3 of the License, or
                ...
                if self._RegionsCal[i][0] < quantity <= self._RegionsCal[i][1]:
                    ...
                    for pos in range(0, 2):
                        if self._RegionsLabel[i][pos].islower():
                            self._list[self._LabelKey[self._RegionsLabel[i][pos].capitalize()]] = float((self._RegionsCal[i][1] - quantity) / base)
                        else:
                            self._list[self._LabelKey[self._RegionsLabel[i][pos].capitalize()]] = float((quantity - self._RegionsCal[i][0]) / base)
        return
...
if __name__ == "__main__":
    ...
    if len(_ab._sys.argv) == 5 or len(_ab._sys.argv) == 7:
        ...
        for i in range(0, 1):
            ...
            _ap = FGPFPMiner(_ab._sys.argv[1], _ab._sys.argv[2], _ab._sys.argv[3], _ab._sys.argv[4], _ab._sys.argv[5], _ab._sys.argv[6])
            _ap.startMine()
            df = pd.DataFrame([algorithm, _ab._sys.argv[4], len(_ap.getPatterns()), _ap.getRuntime(), _ap.getMemoryRSS(),
