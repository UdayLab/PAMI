...
class Spade(_ab._sequentialPatterns):
    ...
    def findTwoLength(self):
        """
            To make 2 length frequent patterns by joining two one length patterns by breadth-first search technique  and update xlenDatabase to seqential database
        """
        self._xLenDatabase = {}
    ...
    def findThreeLength(self):
        ...
        for i in self._xLenDatabase[2].keys():
            for k in self._xLenDatabase[2][i].keys():
                self.generatePatterns(2, i, k)
        for i in self._xLenDatabaseSame[2].keys():
            for k in self._xLenDatabaseSame[2][i].keys():
                self.generatePatternsSame(2, i, k)

    def generatePatterns(self, rowLen, bs, latestWord):
        """
            ...
            :param bs : patterns without latest one
                ...
                if len(self._xLenDatabase[rowLen][bs][latestWord].keys()) <= len(self._xLenDatabase[rowLen][bs][latestWord2].keys()):
                    ...
                    if len(next) >= self._minSup:
                        ...
                        if str(nextRow) not in self._finalPatterns.keys():
                            ...
                            self.generatePatterns(rowLen+1, nextbs, latestWord2)
                    if len(next2) >= self._minSup:
                        ...
                        if str(nextRow) not in self._finalPatterns.keys():
                            ...
                            self.generatePatterns(rowLen+1, nextbs, latestWord)
                    if len(nextSame) >= self._minSup:
                        ...
                        if str(nextRow) not in self._finalPatterns.keys():
                            ...
                            self.generatePatternsSame(rowLen+1, nextbs, nextlste)
                    ...
                    if len(nextSame) >= self._minSup:
                        ...
                        if str(nextRow) not in self._finalPatterns.keys():
                            ...
                            self.generatePatternsSame(rowLen+1, nextbs, nextlate)
    ...
    def generatePatternsSame(self, rowLen, bs, latestWord):
        """
        ...
        :param bs : previous patterns without latest one
    ...
    def makeNextRow(self, bs, latestWord, latestWord2):
        """
            ...
            :param bs : previous pattern without latest one
